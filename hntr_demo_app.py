...
def calculate_blix_score(df):
    ...
    # Normalize values by dividing by maximum (add small constant to avoid division by zero)
...
def perform_clustering(df, n_clusters=3):
    ...
    """
    # Always compute these columns (they are dynamic and not expected in the input)
    df = calculate_blix_score(df)
    df = calculate_fit_score(df)
    
    # Prepare features and fill any missing values with 0
...
def process_data(file):
    ...
    # Inform user that BLIX, Fit, and Priority scores will be created dynamically
